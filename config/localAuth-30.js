...
localAuth.deserializeUser(function(id, done) {
  Clinic.findById(id, function(err, user) {
    if (err) done(err);
    if (user) {
      Clinic.findOneAndUpdate({_id:user.id},{$set:{loggedOut:true,loggedIn:false}},function(err,user){
        if(err) done(err);
        done(null, user);
      });
    } else if(!user) {
      Doctor.findById(id, function(err, user) {
        if (err) done(err);
        if (user) {
          Doctor.findOneAndUpdate({_id:user.id},{$set:{loggedOut:true,loggedIn:false}},function(err,user){
           if(err) done (err);
           done(null, user);
         });
        } else if(!user){
          User.findById(id, function(err, user) {
            if (err) done(err);
            if(user){
              User.findOneAndUpdate({_id:user.id},{$set:{loggedOut:0,loggedIn:1}},function(err,user){
                if(err) done(err);
                done(null, user);
              });
            }
...
Doctor.findOneAndUpdate({'email':email},{$set:{loggedIn:true,loggedOut:false}},function(err,doctor){
  if (err) return done(err);
...
console.log(email,{$set:{loggedIn:true,loggedOut:false}});

User.findOneAndUpdate({'_id':"583d8293cce46431356e70ad"},
  {$set:{loggedOut:false,loggedIn:false}},function(err,user){
    if(err) done(err);
    done(null, user);
    console.log(user);
...
// User.findOneAndUpdate({'_id':"583d8293cce46431356e70ad"},{$set:{loggedIn:true,loggedOut:false}},function(err,myUser){
//   console.log(myUser);
//   if (err) return done(err);
